 ...
 * @details
 * A linked list is a data structure used to store a sequence of
 * values that can be added, deleted, and displayed.
 *
 * ### Algorithm
 * Values can be added by iterating to the end of the list
 * (following the pointers), starting from the first reference.
 * The reference that points to null is considered the
 * last one and points to the new value.
 *
 * Values can also be deleted by iterating through the list.
 * When a node containing a value is found,
 * the node pointing to the current node becomes a pointer to the node pointed to
 * by the current node, and then the current node is returned to the heap store.
 */
#include <iostream>
#include <memory>
#include <string>
