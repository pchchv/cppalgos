/**
 * @file
 * @brief Simple C++ implementation of the SHA-1 Hashing Algorithm
 * Reference: https://en.wikipedia.org/wiki/SHA-1
 *
 * @details
 * SHA-1 is a cryptographic hash function that was developped by the NSA 1995.
 ...
 * To do this, 0x80 (10000000) is added first and then only zeros to
 * the last 8 bytes where the 64-bit size of the input data is added.
 *
 * Once this is done, the algorithm splits this message
 * with padding into chunks of 64 bytes each.
 * Each chunk is used for one *round*,
 * the round splits the chunk into 16 blocks of 4 bytes.
 * These 16 blocks are then expanded to 80 blocks using XOR operations on
 * existing blocks (see code for details).
 * The algorithm then updates its 160-bit state
 * (5 32-bit integers are represented here)
 * using partial hashes computed by special functions on the previously constructed blocks.
 */
