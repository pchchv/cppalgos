 ...
 * @details
 * MD5 is one of the most used hashing algorithm there is.
 * Some of its use cases are:
 *  1. Providing checksum for downloaded software
 *  2. Store salted password
 *
 * MD5 is cryptographically weak, but it is still widely used.
 *
 * ### Algorithm
 * First of all, all values must be in little endian format
 * (details: https://en.wikipedia.org/wiki/Endianness).
 * This is especially important when using part of a byte string as an integer.
 *
 * The first step of the algorithm is to fill the message so
 * that its length is a multiple of 64 (bytes).
 * To do this, 0x80 (10000000) is added first and then
 * only zeros to the last 8 bytes where the 64-bit size of the input data is added.
 *
 * The algorithm then splits this message with padding into chunks of 64 bytes each.
 * Each chunk is used for one *round*, the round splits the chunk into 16 blocks of 4 bytes.
 * During these rounds, the algorithm updates its 128-bit state (represented by 4 ints: A, B, C, D).
 * The signature given by MD5 is its 128-bit state after all rounds are finished.
 */
